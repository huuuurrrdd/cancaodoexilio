...
function displayData(wordData, textData, stoplist, lemmasData){
    ...
    const textsDataset = {
        label: isCategoriaPalavra ? `Número de textos` : `Número de textos:`,
        data: freq_grafico,
        borderWidth: 1,
        borderColor: '#223F29',
    ...
    // Calculate the max value for consistent scale
    let maxValue = Math.max(...freq_grafico)
    if(isCategoriaPalavra) {
        maxValue = Math.max(maxValue, ...freq_palavras_grafico)
    }
    ...
    const chartConfig = {
        ...
        options: {
            ...
            scales: {
                y: {
                    beginAtZero: true,
                    max: maxValue + Math.ceil(maxValue * 0.1), // Add 10% padding
                    position: 'left',
                    title: {
                        display: true,
                        text: isCategoriaPalavra ? 'Valores' : 'Número de textos'
                    }
                }
            }
        }
    ...
    if(isCategoriaPalavra) {
        ...
        <label class="chart-option chart-option2">
            <input type="checkbox" id="show-frequency" checked> 
            <span>Frequência total</span>
        </label>`

        //checked mas n funcional
