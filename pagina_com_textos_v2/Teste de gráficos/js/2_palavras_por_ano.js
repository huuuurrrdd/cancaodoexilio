...
//configuração de objeto para analise flexivel
const config = {
    timeRange: {start: 1846, end: 2025},
    timeGrouping: 'year', // ou decada
    frequencyMetric: 'raw', // ou 'perTotaltexts'(percentagem de textos)
    excludeGroups: ['stopwords', 'original'], // ou 'stopwords' e 'original'
    useLog: false // aplicação de escala logaritima (pode ser definido no grafico)
}
...
function buildWordYearData(wordObj, textData, timeRange){
    ...
    // inicia todos os anos com 0
    for(let year = timeRange.start; year <= timeRange.end; year++){
        yearData[year] = { freq:0, nTexts:0, nome:wordObj.palavra, hasWord:false }
    }
    ...
    wordObj.textos.forEach(t => {
        ...
        if(!textsById[year]) textsById[year] = new Set()
        textsById[year].add(t.id_text)

        if(yearData[year]){
            yearData[year].freq += t.frequencia

            if(t.frequencia > 0){
                yearData[year].hasWord = true // marca que a palavra aparece neste ano
            }  
        }
    })
...
//calcula total de palavras por ano por todos os anos
...
// Criação de sets de indices de palavras para excluir
function createExclusionSets(wordData, stopList, originalWords, excludeGroups){
    const sets = {
        stopwords: new Set(),
        original: new Set()
    }
    ...
    //combina sets para exclusão baseado na configuracao
...
//Agrupamaneto de anos em periodos de tempo (anos ou decadas)
...
//TOU NA DUVIDA - DEVIA CONTAR A PALAVRA UMA VEZ POR CADA PERIODO
// calcula a metrica da frequencia com base na configuracao
function calculateMetric(freq, nTexts, totalWords, config){
    let value = 0

    switch (config.frequencyMetric){
        case 'perText':
            value = nTexts > 0 ? freq / nTexts : 0
            break
        case 'perTotalWords':
            value = totalWords > 0 ? freq / totalWords : 0
            break
        case 'raw':
        default:
            value = freq
    ...
    return config.useLog && value > 0 ? Math.log(value) : value
}

//junta frequencia de palavras com base em periodo de tempo (anos ou decadas) //N FAZ SENTIDO FALAR EM FREQUENCIA AQUI!?
// agora palavras unicas em vez de encontrar a mais popular
function aggregateByTimePeriod(wordFrequencies, wordData, exclusionSet, config, totalWordsPerYear){
    ...
    //Conta palavras unicas para cada periodo /// AQUI JÁ FAZ SENTIDO, MAS N ESTARÁ DEMASIADO COMPLEXO??
    periods.forEach((_, period) => {
        ...
        for(let wordIdx = 0; wordIdx < wordData.palavras.length; wordIdx++){
            ...
            let wordAppearsInPeriod = false

            // verifica se palavra aparece em algum ano deste periodo
            for(let year = config.timeRange.start; year <= config.timeRange.end; year++){
                if(getTimePeriod(year, config.timeGrouping) === period){
                    const yearData = wordFrequencies[wordIdx][year]
                    if(yearData.hasWord){
                        wordAppearsInPeriod = true
                        break
                    }
                }
            }

            if(wordAppearsInPeriod){
                uniqueWordCount++
                uniqueWords.push(wordData.palavras[wordIdx].palavra)
            }
        }
...
// renderizar o gráfico com os dados agrupados
function renderChart(aggregatedData, config){
    ...
    title.textContent = `Número de palavras presentes em cada ${config.timeGrouping}`
    ...
    subtitle.textContent = `Agrupamento: ${config.timeGrouping}, Exclusões: ${config.excludeGroups.join(', ')}`
    ...
    new Chart(canvas, {
        ...
        data: {
            labels:labels,
            datasets: [{
                label: 'Número de palavras presentes em cada periodo',
                data: data,
                borderWidth: 1,
...
//muda de configuração e volta a correr
function updateConfig(newConfig){

    Object.assign(config, newConfig)

    //limpa os gráficos existentes e volta a renderizar
    ...
    if(globalWordData && globalTextData && globalStoplist){
        ...
        console.error('Dados não foram carregados. Chamar fetchData() primeiro')
    }
}
...
function criaBotoesControlo(){
    ...
    title.textContent = 'Botões de análise'
